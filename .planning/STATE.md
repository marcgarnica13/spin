...
| 260817-fast | Auto-assign distinct Claude prompt-bar color per window via `/color` slash command (palette round-robin by window index, boot-watcher via capture-pane); replaces the reverted tmux tab coloring | 2026-08-17 | c0517d5 | (inline fast task, no directory) |
